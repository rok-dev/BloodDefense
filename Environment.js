(function(window) {
    ...
    /**
     * This tutorial was used as a basis for this function and createBloodVesselPart():
    ...
    Environment.prototype.drawHills = function(numberOfHills, pixelStep) {
        var hillStartY = 250 + (Math.random() - 0.5) * 50;
        ...
        for (var i = -1; i < numberOfHills; i++) {
            var randomHeight = (Math.random() + 0.5) * 50;
            if (i != -1) hillStartY -= randomHeight;
            //for (var j = 0; j <= hillSliceWidth; j++) {
            for (var j = 0; j < hillSliceWidth; j++) {

                createBloodVesselPart(j, pixelStep, hillWidth, i, hillStartY, randomHeight, hillSliceWidth, upBloodWall, downBloodWall);
            }
            if (i == numberOfHills - 1) {
                createBloodVesselPart(hillSliceWidth, pixelStep, hillWidth, i, hillStartY, randomHeight, hillSliceWidth, upBloodWall, downBloodWall);
            }
    ...
    function createBloodVesselPart(j, pixelStep, hillWidth, i, hillStartY, randomHeight, hillSliceWidth, upBloodWall, downBloodWall) {
